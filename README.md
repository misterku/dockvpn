...
```bash
docker build . -t kusevich
```


```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp kusevich)
docker run -t -i -p 8080:8080 --volumes-from $CID kusevich serveconfig
```

Or run without logs:

```bash
CID=$(docker run -d --privileged -p 1194:1194/udp -p 443:443/tcp -e "DISABLE_LOGS=1" --log-driver none kusevich)
docker run -t -i -p 8080:8080 -e "DISABLE_LOGS=1" --log-driver none --volumes-from $CID kusevich serveconfig
```
